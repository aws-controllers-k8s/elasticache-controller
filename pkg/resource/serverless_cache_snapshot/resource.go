...
func (r *resource) SetIdentifiers(identifier *ackv1alpha1.AWSIdentifiers) error {
	...
	r.ko.Spec.ServerlessCacheSnapshotName = &identifier.NameOrID

	f2, f2ok := identifier.AdditionalKeys["serverlessCacheName"]
	if f2ok {
		r.ko.Spec.ServerlessCacheName = aws.String(f2)
	}
	f4, f4ok := identifier.AdditionalKeys["snapshotType"]
	if f4ok {
		r.ko.Status.SnapshotType = aws.String(f4)
	}
